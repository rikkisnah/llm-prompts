...
.PHONY: help setup format test test-% lint score score-gate check validate clean clean-env sync list dry-run-delete
...
help:
	...
	@printf '  %-14s %s\n' 'sync' 'Install skills/ into ~/.claude/skills and ~/.codex/skills.'
	@printf '  %-14s %s\n' 'list' 'List source and installed skills.'
	@printf '  %-14s %s\n' 'dry-run-delete' 'Preview removing SKILL=<name> from installed skill dirs.'
...
test:
	@if [ -d tests ]; then PYTHONPATH=src $(PYTHON) -m unittest discover -s tests -v; else echo 'No tests directory found.'; fi

test-%:
	@if [ -f "tests/test_$*.py" ]; then PYTHONPATH=src $(PYTHON) -m unittest "tests.test_$*"; \
	elif [ -f "tests/$*.py" ]; then module=$$(printf '%s' "$*" | sed 's#/#.#g; s#\.py$$##'); PYTHONPATH=src $(PYTHON) -m unittest "tests.$$module"; \
...
sync:
	PYTHONPATH=src $(PYTHON) -m skills_manager sync --home --skills $(SKILLS_DIR)

list:
	PYTHONPATH=src $(PYTHON) -m skills_manager list --home --skills $(SKILLS_DIR)

dry-run-delete:
	@test -n "$(SKILL)" || { echo 'ERROR: pass SKILL=<skill-name>'; exit 1; }
	PYTHONPATH=src $(PYTHON) -m skills_manager delete "$(SKILL)" --home --skills $(SKILLS_DIR) --dry-run
